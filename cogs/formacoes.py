...
def _formatar_numero_sequencial(numero: int | None) -> str:
    """'Nº72' quando tem número, 'S/Nº' quando não tem — evita o feio
    'NºNone' que aparecia antes pra formações sem número sequencial."""
    return f"Nº{numero}" if numero is not None else "S/Nº"
...
class Formacoes(commands.Cog):
    ...
    async def consultar(self, interaction: discord.Interaction, roblox_username: str):
        ...
        for f in formacoes:
            ...
            edicao_label = f["edicao_numero"]
            if f["edicao_apelido"]:
                edicao_label += f" {f['edicao_apelido']}"
            linhas.append(
                f"• {f['curso_nome']} — {_formatar_numero_sequencial(f['numero_sequencial'])} — "
                f"Edição: {edicao_label} — {f['data']}{status}"
            )
    ...
    async def relatorio(self, interaction: discord.Interaction, curso: str, edicao: str):
        ...
        for r in registros:
            ...
            linhas.append(f"• {r['roblox_username']} — {_formatar_numero_sequencial(r['numero_sequencial'])}{status}")
