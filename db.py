...
async def buscar_curso_por_nome_exato(nome: str):
    ...
    Normaliza espaços (duplos, ou invisíveis tipo NBSP de copiar-e-colar)
    antes de comparar — sem isso, a mesma thread renomeada com um espaço
    a mais/a menos virava um curso "novo" e duplicado a cada reimportação
    (mesmo bug que já existia em `get_ou_criar_edicao` antes de ser
    corrigido)."""
...
async def get_ou_criar_curso(nome: str, estrito: bool = False) -> int:
    ...
    estrito=True: usa só nome exato (com espaços normalizados) — pra
    importação automática de threads do fórum, onde cada thread já É o
    nome oficial e completo do curso, e a busca difusa causaria fusões
    indevidas entre cursos diferentes."""
    if estrito:
        nome = re.sub(r"\s+", " ", nome).strip()
...
async def documentos_por_unidade(unidade: str, termo: str | None = None, limite: int = 20):
    """Lista só os documentos de uma unidade específica (busca por
    'contém', não precisa ser o nome exato da unidade) — diferente de
    `buscar_documentos`, que também bate o termo contra tipo/número/título
    e pode trazer documentos de OUTRAS unidades que só mencionem o termo
    no título. `termo` é opcional, pra filtrar mais ainda dentro dessa
    unidade (ex: unidade='Regimento de Cavalaria Mecanizado', termo='2024')."""
    async with aiosqlite.connect(config.DATABASE_PATH) as db:
        db.row_factory = aiosqlite.Row
        curinga_unidade = f"%{unidade}%"
        if termo:
            curinga_termo = f"%{termo}%"
            cur = await db.execute(
                "SELECT * FROM documentos WHERE unidade LIKE ? AND "
                "(tipo LIKE ? OR numero LIKE ? OR titulo LIKE ?) "
                "ORDER BY criado_em DESC LIMIT ?",
                (curinga_unidade, curinga_termo, curinga_termo, curinga_termo, limite),
            )
        else:
            cur = await db.execute(
                "SELECT * FROM documentos WHERE unidade LIKE ? ORDER BY criado_em DESC LIMIT ?",
                (curinga_unidade, limite),
            )
        return await cur.fetchall()


async def unidades_autocomplete(termo: str, limite: int = 20):
    """Lista de unidades já usadas em documentos cadastrados que batem com
    o texto digitado até agora — evita erro de digitação ao filtrar por
    unidade (ex: digitar 'Cavalaria' e sugerir o nome completo certinho)."""
    async with aiosqlite.connect(config.DATABASE_PATH) as db:
        cur = await db.execute(
            "SELECT DISTINCT unidade FROM documentos WHERE unidade LIKE ? "
            "ORDER BY unidade LIMIT ?",
            (f"%{termo}%", limite),
        )
