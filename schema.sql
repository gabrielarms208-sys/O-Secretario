...
CREATE INDEX idx_militares_username ON militares(roblox_username);
CREATE INDEX idx_formacoes_militar ON formacoes(militar_id);
CREATE INDEX idx_formacoes_curso_edicao ON formacoes(curso_id, edicao_id);
...
CREATE TABLE historico_fichas_arquivadas (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    militar_id      INTEGER REFERENCES militares(id),
...
CREATE INDEX idx_arquivo_militar ON historico_fichas_arquivadas(militar_id);
...
CREATE INDEX idx_documentos_busca ON documentos(unidade, tipo, numero);
