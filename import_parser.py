...
1) parse_ficha: posts do fórum "ficha-militar" (SGEx) — texto com campos
   tipo "▷Posto: X;" um por linha. (sem mudanças nesta versão)

2) parse_formados: posts dos fóruns de formados (formados-cursos,
   formados-estágios, formados-escolas-academias). Essa versão é mais
   TOLERANTE que a anterior — o formato original exigia exatamente
   "- usuario → Curso Nº12" com cabeçalho "# Edição" antes. Agora ela
...
def parse_ficha(texto: str) -> dict:
    ...
    orgs = _extrair_bloco(texto, r"Organiza[cç][õo]es Militares", r"Dados Funcionais|Forma[cç][õo]es")
    if orgs:
